...
for i, dataset in enumerate(datasets):
    ...
    showlegend = i < 1
    fig.add_trace(go.Line(y=df.train_loss, name="Train", line=dict(color='blue'), showlegend=showlegend), row=r, col=c)
    fig.add_trace(go.Line(y=df.valid_loss, name="Valid.", line=dict(color='red'), showlegend=showlegend), row=r, col=c)
    for j, diff in enumerate(df.diff_loss):
        if diff == 1:
            y0 = min(df.valid_loss.min(), df.train_loss.min())
            y1 = max(df.valid_loss.max(), df.train_loss.max())
            fig.add_shape(go.layout.Shape(type='line', xref='x', yref='y', x0=j, y0=y0, x1=j, y1=y1,
                                          line=dict(color='gray', width=0.8)), row=r, col=c)
